...
/**
 * REST controller for managing {@link de.mayflower.kickit.domain.Contest}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class ContestResource {
    ...
    public ResponseEntity<Contest> updateContest(
        ...
        return ResponseEntity
            .ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, contest.getId().toString()))
            .body(result);
    }
    ...
    public ResponseEntity<Contest> partialUpdateContest(
        ...
        Optional<Contest> result = contestRepository
            ...
            .map(existingContest -> {
                if (contest.getDate() != null) {
                    existingContest.setDate(contest.getDate());
                }
                if (contest.getContestMode() != null) {
                    existingContest.setContestMode(contest.getContestMode());
                }
                if (contest.getLocation() != null) {
                    existingContest.setLocation(contest.getLocation());
                }
                if (contest.getWinnerTeam() != null) {
                    existingContest.setWinnerTeam(contest.getWinnerTeam());
                }
        ...
        return ResponseUtil.wrapOrNotFound(
            result,
            HeaderUtil.createEntityUpdateAlert(applicationName, false, ENTITY_NAME, contest.getId().toString())
        );
    }
